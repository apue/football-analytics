...
resolve rights in the original source. Normalized facts derived from a reviewed
snapshot may be used only with an explicit partial-coverage scope; absence of
an appearance row cannot be interpreted as zero. The repository has no
provider-specific adapter for this source.
